...
public class GEOConvertor extends AbstractEllipsoid {
	...
	/**
	 * Method convert Decimal Coordinate to UTM Coordinate. Return string with X
	 * and Y.
	 * <p>
	 * Passing parameters example: geoToUTM(21.88684199, 47.30414058,
	 * Datum.SAD69, 45);
	 * </p>
	 * 
	 * @param lat
	 *            latitude coordinate (decimal/minute/second).
	 * @param lng
	 *            longitude coordinate (decimal/minute/second).
	 * @param datum
	 *            a reference datum.
	 * @param meridianCentral
	 *            define central meridian.
	 * @return the string UTM coordinates.
	 */

	// Fedor!!! dataum com as constantes ou facada com estrategia em algoritimo
	public String geoToUTM(double lat, double lng, Datum datum,
		...
		if (datum.name().equals("SAD69")) {
			...
			// Coefficients
			S = sCoeff(SAD69.MAJOR_AXIS, lat, e1, aCoeff(e1), bCoeff(e1),
					cCoeff(e1), dCoeff(e1), eCoeff(e1), fCoeff(e1));
			return calcUTM(e1, e2, N, S, meridianCentral);
		} else if (datum.name().equals("WGS84")) {
			// eccentricity
			e1 = eccentricityOne(WGS84.MAJOR_AXIS, WGS84.MINOR_AXIS);
			e2 = eccentricityTwo(WGS84.MAJOR_AXIS, WGS84.MINOR_AXIS);
			// Calculate BigNormal
			N = bigN(WGS84.MAJOR_AXIS, e1, Math.toRadians(lat));
			// Coefficients
			S = sCoeff(WGS84.MAJOR_AXIS, lat, e1, aCoeff(e1), bCoeff(e1),
					cCoeff(e1), dCoeff(e1), eCoeff(e1), fCoeff(e1));
			return calcUTM(e1, e2, N, S, meridianCentral);
		}
		return null;
	 ...
	 * <p>
	 * For example: dmsToDEC(-215312.63115)
	 * </p>
	 * 
	 * @param coord
	 *            insert latitude or longitude (Degrees, Minutes, Seconds).
	 ...
	 * <p>
	 * For example: decToDMS(49.8488438831)
	 * </p>
	 * 
	 * @param decimalDegree
	 *            insert latitude or longitude (Decimal Degree).
	 * @return the string coordinates.
	 */
	public String decToDMS(double decimalDegree) {
		String degrees = String.valueOf(Math.abs(decimalDegree)).substring(0, 2);
